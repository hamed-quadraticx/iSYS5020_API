...
class EhternetInterface:
    def __init__(self,radar_obj,enable_log=False):
        ...
        _address = (radar_obj.IP_address,radar_obj.destination_targetlist_portno)
        self.udp_socket.bind(_address)
        logging.info(f'radar udp socket binding to {radar_obj.IP_address} : {radar_obj.destination_targetlist_portno}')
    ...
    def parsing_TargetList(self):
        ...
        if chk_valid_header and header_obj.no_of_packets > 0 and header_obj.no_of_targets > 0 :
            ...
            data_packet = Target_Data_Packet(header_obj.frame_id,header_obj.no_of_packets, header_obj.no_of_targets, data)
            data_packet.parse()
