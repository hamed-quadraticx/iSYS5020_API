...
class  StopAcq_req(ReqMessage):
   def __init__(self,radar_obj):
      ...
      self.FC = [0xD1,0x00,0x01]
      self.FCS = 0x00
      self.ED = 0x16
    
   def validate_received_rawdata(self,msg_arr):
      getack_response_obj = StopAcq_response(self.radar_obj,msg_arr)
      print('waiting stop acqusition.....')
      is_valid =  getack_response_obj.validate_msg()
      if is_valid is False:
          print('invalid stop acq ack  message')
          return False
...
############################################## class for response ########################
class StopAcq_response(ResponseMessage):
    def __init__(self,radar_obj,msg):
      ...
      if len(msg) == 9 :
         ...
         self.msg_arr = msg[37:46]

      print('stop valid ack is :')
      print (' , '.join(format(x, '02x') for x in self.msg_arr))
      self.SD = 0x68
      self.LE = 0x03
      self.LEr =0x03
    ...
    def validate_msg(self):
      ...
      print('received a valid stop  Acq Ack message !!')
      return True
